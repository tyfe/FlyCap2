...
UINT RecordingDialog::ThreadSaveFrameHelper()
{
	...
	if (m_tabCtrl_OutputType.GetCurSel() == OUTPUT_TYPE_IMAGE)
	{
		while (GetRecorderState() == STARTED)
		{
			...
			bufferLock.Lock();
			RetrieveNextImageFromBuffer(&image, &s);
			...
			try
			{
				SaveImageToFile(&image, &m_imageSettings, &s);
			}
...
void RecordingDialog::StartRecording()
{
	...
	// delay so as to facilitate getting the camera ready
	Sleep(20000);
...
Error RecordingDialog::SaveImage(Image* tmp, ImageRecordingPage::ImageSettings* imageSettings, unsigned int count, std::string * timestamp)
{
	char saveName[MAX_PATH];

	sprintf(saveName, "%s-%s-%04d.%s", imageSettings->filename, timestamp->c_str(), count, imageSettings->fileExtension);
...
void RecordingDialog::RetrieveNextImageFromBuffer(Image* image, std::string* rtime)
{
	if (m_grabMode == BUFFER_FRAMES)
	{
		*image = m_imageBuffer.front();
		*rtime = m_timeBuffer.front();
	...
	else
	{
		...
		*rtime = m_timeBuffer.back();
		m_imageBuffer.clear();
		m_timeBuffer.clear();
	}
}

void RecordingDialog::SaveImageToFile(Image* image, ImageRecordingPage::ImageSettings* imageSettings, std::string * timestamp)
{
	Error error;
		...
		(pixelFormat == PIXEL_FORMAT_RAW16))
	{
		...
		if ((colAlg != NO_COLOR_PROCESSING) && !m_isSaveBayerImageSelected)
		{
			...
			{
				error = SaveImage(&convertedImage, imageSettings, m_savedImageCounter, timestamp);
			}
		}
		else
		{
			error = SaveImage(image, imageSettings, m_savedImageCounter, timestamp);
		}
	}
	else
	{
