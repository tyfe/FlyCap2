...
protected:
	...
	void RetrieveNextImageFromBuffer(Image * image, std::string * rtime);

	void StartRecording();
	void StoppingRecording();
	void ProcessCurrentFrame(Image* rawImage);
	Error SaveImage(Image* tmp, ImageRecordingPage::ImageSettings* imageSettings, unsigned int count, std::string * timestamp);
	...
	void SaveImageToFile(Image* image, ImageRecordingPage::ImageSettings* imageSettings, std::string * s);
...
protected:
	...
	std::vector<std::string> m_timeBuffer;
